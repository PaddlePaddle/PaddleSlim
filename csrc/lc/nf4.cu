...
__global__ void kDequantizeBlockwiseNF4(const unsigned char * A, const float * absmax, T *out, int blocksize, const int n)
{
...
#define MAKE_kDequantizeBlockwiseNF4(dtype, tile_size, threads, num_per_thread) \
template __global__ void kDequantizeBlockwiseNF4<dtype, tile_size, threads, num_per_thread>(const unsigned char * A, const float * absmax, dtype *out, int blocksize, const int n); \

MAKE_kDequantizeBlockwiseNF4(half, 512, 64, 8)
MAKE_kDequantizeBlockwiseNF4(__nv_bfloat16, 512, 64, 8)
MAKE_kDequantizeBlockwiseNF4(float, 512, 64, 8)


template <paddle::DataType D>
std::vector<paddle::Tensor>  LaunchDeQuantizeNF4(const paddle::Tensor& input,
                                                const paddle::Tensor& absmax,
                                                int block_size) {
  typedef PDTraits<D> traits_;
  typedef typename traits_::DataType DataType_;
  typedef typename traits_::data_t data_t;
  
  auto output = paddle::full(input.shape(), 1, traits_::dtype_s, input.place());
  
  const unsigned char *in_ptr = input.data<unsigned char>();
  const float *abs_max_ptr = absmax.data<float>();  
  DataType_ *out_ptr = reinterpret_cast<DataType_*>(output.mutable_data<data_t>());
  // 修改上句
  ...
  kDequantizeBlockwiseNF4<DataType_, 512, 64, 8><<<(n+tile_size-1)/tile_size, 64>>>(in_ptr, abs_max_ptr, out_ptr, block_size/2, n);
  return {output};
}
...
std::vector<paddle::Tensor> DeQuantizeNF4(const paddle::Tensor& input,
                                          const paddle::Tensor& absmax,
                                          int block_size, 
                                          int dtype) {
    ...
    switch (dtype) {
        case 0: {
            return LaunchDeQuantizeNF4<paddle::DataType::BFLOAT16>(input, absmax, block_size);
        }
        case 1: {
            return LaunchDeQuantizeNF4<paddle::DataType::FLOAT16>(input, absmax, block_size);
        }
        case 2: {
            return LaunchDeQuantizeNF4<paddle::DataType::FLOAT32>(input, absmax, block_size);
        }
        default: {
            PD_THROW(
                "NOT supported data type. "
...
PD_BUILD_OP(dequantize_nf4)
    .Inputs({"input", "absmax"})
    .Outputs({"out"})
    .Attrs({"block_size: int", "dtype: int"})
    .SetKernelFn(PD_KERNEL(DeQuantizeNF4));