...
def reader():
    ...
    tokenizer = BertTokenizer.from_pretrained(
        '/root/work/source-free/PaddleSlim/demo/auto-compression_origin/static_bert_models/'
    )  ###args.model_dir)
    ...
    [input_ids, token_type_ids, labels] = create_data_holder(
        'sst-2')  ###args.task)
    feed_list_name = []
...
if __name__ == '__main__':
    ...
    ### get all patterns
    patterns, graph, distill_node = get_patterns(inference_program,
                                                 'transformer')
    for fetch_node in fetch_targets:
        distill_node.append('teacher_' + fetch_node.name)
        distill_node.append(fetch_node.name)
    print(distill_node)
