...
def main():
    ...
    place = paddle.CUDAPlace(FLAGS.gpu,
                             ) if FLAGS.devices == 'gpu' else paddle.CPUPlace()
    ...
    inference_model_path = config["model_dir"].rstrip().rstrip('.onnx',
                                                               ) + '_infer'
    ...
    quant_recon_static(
        ...
        recon_grit=config['recon_grit'],
        is_intro_act_noise=False,
        blocks=config['blocks'],
        block_weights_names=config['block_weights_names'],
