...
class ReActConv2d(nn.Layer):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, bias=False, dilation=0, transposed=False, output_padding=None, groups=1):
        ...
        self.channel_threshold = paddle.create_parameter(shape=[1, in_channels, 1, 1], dtype="float32", default_initializer=nn.initializer.Assign(paddle.zeros([1, in_channels, 1, 1])))

    def forward(self, x):
        x = x + self.channel_threshold
        out_forward = paddle.sign(x)
        mask1 = x < -1
        mask2 = x < 0
        mask3 = x < 1
        out1 = (-1) * paddle.cast(mask1, dtype=paddle.float32) + (x*x + 2*x) * (1-paddle.cast(mask1, dtype=paddle.float32))
        out2 = out1 * paddle.cast(mask2, dtype=paddle.float32) + (-x*x + 2*x) * (1-paddle.cast(mask2, dtype=paddle.float32))
        out3 = out2 * paddle.cast(mask3, dtype=paddle.float32) + 1 * (1- paddle.cast(mask3, dtype=paddle.float32))
