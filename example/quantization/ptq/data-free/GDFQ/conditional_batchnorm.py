...
class ConditionalBatchNorm2d(nn.BatchNorm2D):
    ...
    def __init__(self, num_features, eps=1e-05, momentum=0.1,
                 affine=False, track_running_stats=None):
        super(ConditionalBatchNorm2d, self).__init__(
            num_features, epsilon=eps, momentum=momentum, use_global_stats=track_running_stats
        )
        self.training = affine
    ...
    def forward(self, input, weight, bias, **kwargs):
        ...
        output = F.batch_norm(input, self._mean, self._variance,
                              self.weight, self.bias,
                              self.training,
                              momentum=exponential_average_factor, epsilon=self._epsilon)
        if weight.dim() == 1:
...
class CategoricalConditionalBatchNorm2d(ConditionalBatchNorm2d):
    ...
    def _initialize(self):
        param_init.constant_init(self.weights.weight, value=1.0)
        param_init.constant_init(self.biases.weight, value=0.0)
