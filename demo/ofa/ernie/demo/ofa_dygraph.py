...
if __name__ == '__main__':
    ...
    parser.add_argument('--inference_model_dir', type=str, default='ofa_ernie_inf', help='inference model output directory')
    parser.add_argument('--save_dir', type=str, default='ofa_ernie_save', help='model output directory')
    ...
    with FD.guard(place):
        ...
        default_run_config = {'train_batch_size': args.bsz, 'eval_batch_size': args.bsz, 'n_epochs': [[4 * args.epoch], [6 * args.epoch]], 'init_learning_rate': [[args.lr], [args.lr]], 'elastic_depth': [2, 3], 'total_images': 1, 'dynamic_batch_size': [[1, 1], [1, 1]]}
        ...
        if args.use_lr_decay:
            opt = AdamW(learning_rate=LinearDecay(args.lr, int(args.warmup_proportion * args.max_steps), args.max_steps), parameter_list=ofa_model.parameters(), weight_decay=args.wd, grad_clip=g_clip)
        else:
            opt = AdamW(args.lr, parameter_list=ofa_model.parameters(), weight_decay=args.wd, grad_clip=g_clip)

        for epoch in range(2 * args.epoch):
            ...
            for step, d in enumerate(tqdm(train_ds.start(place), desc='training')):
                ...
                if step % 100 == 0:
                    ...
                    print('acc %.5f, teacher acc %.5f' % (np.concatenate(acc).mean(), np.concatenate(tea_acc).mean()))
        if args.save_dir is not None:
            F.save_dygraph(ofa_model.state_dict(), args.save_dir)
