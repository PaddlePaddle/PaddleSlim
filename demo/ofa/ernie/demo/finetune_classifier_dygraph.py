...
if __name__ == '__main__':
    ...
    parser.add_argument('--inference_model_dir', type=str, default='ernie_w1', help='inference model output directory')
    parser.add_argument('--save_dir', type=str, default='ernie_w1_save', help='model output directory')
    ...
    with FD.guard(place):
        ...
        for epoch in range(args.epoch):
            for step, d in enumerate(tqdm(train_ds.start(place), desc='training')):
                ...
                loss.backward()
                if step % 10 == 0:
                    log.info('train loss %.5f lr %.3e' % (loss.numpy(), opt.current_step_lr()))
                opt.minimize(loss)
