...
BlockConfig = namedtuple('BlockConfig', 'stride, num_outputs, preact_bn, block_type')
...
class PVANet():
    ...
    def net(self, input, include_last_bn_relu=True, class_dim=1000):
        ...
        pool1 = fluid.layers.pool2d(input=conv1, pool_size=3, pool_stride=2, pool_padding=1, pool_type='max', name='pool1')

        end_points = {}
        conv2 = self._conv_stage(pool1,
                block_configs=[
        ...
        conv4 = self._conv_stage(conv3,
                block_configs=[
                    BlockConfig(2, '64 48-96 24-48-48 96 128', True, BLOCK_TYPE_INCEP),
        ...
        conv5 = self._conv_stage(conv4,
                block_configs=[
                    BlockConfig(2, '64 96-128 32-64-64 128 196', True, BLOCK_TYPE_INCEP),
        ...
        if include_last_bn_relu:
            conv5 = self._bn(conv5, 'relu', 'conv5_4/last_bn')
    ...
    def _conv_stage(self, input, block_configs, name, end_points):
        ...
        for idx, bc in enumerate(block_configs):
            if bc.block_type == BLOCK_TYPE_MCRELU:
                ...
                block_scope = '{}_{}/incep'.format(name, idx + 1)
    ...
    def _mCReLU(self, input, mc_config, name):
        ...
        if mc_config.preact_bn:
            ...
            conv1_fn = self._conv
            conv1_scope = name + '/1/conv'

        sub_conv1 = conv1_fn(input, mc_config.num_outputs[0],
                1, conv1_scope, mc_config.stride)

        sub_conv2 = self._bn_relu_conv(sub_conv1,
                mc_config.num_outputs[1], 3, name + '/2')

        sub_conv3 = self._bn_crelu_conv(sub_conv2,
                mc_config.num_outputs[2], 1, name + '/3')

        if int(input.shape[1]) == mc_config.num_outputs[2]:
            conv_proj = input
        else:
            conv_proj = self._conv(input, mc_config.num_outputs[2],
                    1, name + '/proj', mc_config.stride)
    ...
    def _inception_block(self, input, block_config, name):
        ...
        for path_idx, path_outputs in enumerate(num_outputs[1:]):
            ...
            path_scopes.extend(['/{}_{}'.format(path_idx, i - 1) for i in range(1, len(path_outputs))])
        ...
        if block_config.preact_bn:
            preact = self._bn(input, 'relu', name + '/bn')
        else:
            preact = input
        ...
        for path_idx, path_param in enumerate(path_params):
            path_net = preact
            for conv_idx, (num_output, scope, kernel_size) in enumerate(zip(*path_param)):
                if conv_idx == 0:
                    conv_stride = stride
                else:
                    conv_stride = 1
                path_net = self._conv_bn_relu(path_net, num_output,
                        kernel_size, name + scope, conv_stride)
            paths.append(path_net)

        if stride > 1:
            path_net = fluid.layers.pool2d(input, pool_size=3, pool_stride=2,
                    pool_padding=1, pool_type='max', name=name + '/pool')
            path_net = self._conv_bn_relu(path_net, pool_path_outputs, 1, name + '/poolproj')
            paths.append(path_net)
        block_net = fluid.layers.concat(paths, axis=1)
        block_net = self._conv(block_net, inception_outputs, 1, name + '/out/conv')

        if int(input.shape[1]) == inception_outputs:
            proj = input
        else:
            proj = self._conv(input, inception_outputs, 1, name + '/proj', stride)
        return block_net + proj

    def _scale(self, input, name, axis=1, num_axes=1):
        assert num_axes == 1, "layer scale not support this num_axes[%d] now" % (num_axes)
    ...
    def _bn(self, input, act, name):
        net = fluid.layers.batch_norm(input=input,
                act=act,
                name=name,
    ...
    def _bn_relu_conv(self,
        ...
        net = self._bn(input, 'relu', name + '/bn')
        net = self._conv(net, num_filters, filter_size, name + '/conv', stride, groups)
        return net
    ...
    def _conv_bn_relu(self,
                      ...
                      stride=1,
                      groups=1):
        net = self._conv(input, num_filters, filter_size, name + '/conv', stride, groups)
        net = self._bn(net, 'relu', name + '/bn')
        return net

    def _bn_crelu(self, input, name):
        net = self._bn(input, None, name + '/bn_1')
        neg_net = fluid.layers.scale(net, scale=-1.0, name=name + '/neg')
        net = fluid.layers.concat([net, neg_net], axis=1)
        net = self._scale(net, name + '/scale')
        net = fluid.layers.relu(net, name=name + '/relu')
        return net
    ...
    def deconv_bn_layer(self,
        ...
        deconv = fluid.layers.conv2d_transpose(
            ...
            bias_attr=ParamAttr(name=name + '_bias'),
            name=name + '/deconv')
        return self._bn(deconv, act, name + '/bn')
    ...
    def conv_bn_layer(self,
                    ...
                    stride=1,
                    groups=1):
        return self._conv_bn_relu(input, num_filters, filter_size, name, stride, groups)
    ...
    for i in range(len(f)-1):
        if i == 0:
            g[i] = net.deconv_bn_layer(h[i], num_outputs[i], name='fpn_0')
             ...
             out = net.conv_bn_layer(out, num_outputs[i], 1, 'fpn_trans_' + str(i))
             g[i] = net.deconv_bn_layer(out, num_outputs[i], name='fpn_' + str(i))
...
def Detector_Header(f_common, net, class_num):
    ...
    f_geo = fluid.layers.conv2d(f_geo, 8, 1, use_cudnn=True,
            param_attr=ParamAttr(name='geo_4/conv_weights'),
            bias_attr=ParamAttr(name='geo_4/conv_bias'),
            name='geo_4/conv')
    ...
    f_score = fluid.layers.conv2d(f_score, class_num+1, 1, use_cudnn=True,
            param_attr=ParamAttr(name=name + '/conv_weights'),
            bias_attr=ParamAttr(name=name + '/conv_bias'),
            name=name + '/conv')
...
def east(input, class_num=31):
    ...
    out = net.net(input)
    blocks = []
    for i, j, k in zip(['conv2','conv3','conv4','conv5'], [1,2,4,8], [64, 64, 64, 64]):
        if j == 1:
            conv = net.conv_bn_layer(out[i], k, 1, name='fusion_'+str(len(blocks)))
        elif j <= 4:
            conv = net.deconv_bn_layer(out[i], k, 2*j, j, j//2, name='fusion_'+str(len(blocks)))
        else:
            conv = net.deconv_bn_layer(out[i], 32, 8, 4, 2, name='fusion_'+str(len(blocks))+'/1')
            conv = net.deconv_bn_layer(conv, k, j//2, j//4, j//8, name='fusion_'+str(len(blocks))+'/2')
        blocks.append(conv)
...
def loss(f_score, f_geo, l_score, l_geo, l_mask, class_num=1):
    ...
    #smooth_l1_loss
    channels = 8
    l_geo_split, l_short_edge = fluid.layers.split(l_geo, num_or_sections=[channels, 1], dim=1) #last channel is short_edge_norm
    ...
    l_short_edge = fluid.layers.expand(x=l_short_edge, expand_times=[1, channels, 1, 1])
