...
add_arg('warm_epochs',       int, 20,                    "the training epoch number before pruning")
add_arg('pruning_period',    int, 20,                    "the period to increase pruning ratio")
...
def compress(args):
    ...
    ratio_step = (args.num_epochs - args.warm_epochs) / args.pruning_period

    for epoch_id in range(args.num_epochs):
        if epoch_id >= args.warm_epochs and epoch_id % args.pruning_period == 0:
            pruner.ratio += ratio_step
