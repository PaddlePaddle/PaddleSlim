...
add_arg('test_period',      int, 2,                 "Test period in epoches.")
add_arg('model_path',       str, "./models",         "The path to save model.")
add_arg('model_period',     int, 10,             "The period of model saving.")
add_arg('min_ratio',        float, 0.4,             "The starting pruning ratio corresponding to the starting epoch.")
add_arg('max_ratio',        float, 0.6,             "The maximum pruning ratio.")
add_arg('resume_epoch',     int, 0,             "The starting epoch.")
...
def debug_helper(program):
    helper = {}
    for param in program.all_parameters():
        tmp = paddle.static.global_scope().find_var(param.name).get_tensor()
        helper[param.name] = np.mean(np.array(tmp))
    return helper
...
def compress(args):
    ...
    if args.data == "mnist":
        ...
        transform = T.Compose(
            [T.Transpose(), T.Normalize([127.5], [127.5], [127.5])])
    ...
    #NOTE 1: initialize the unstructure pruner
    ...
    def test(epoch, program):
        acc_top1_ns = []
        acc_top5_ns = []
        #NOTE 6: print the sparsity of the test model
        ...
        for batch_id, data in enumerate(valid_loader):
            tmp = np.array(data[0]['label'])
            if not (len(tmp.shape) == 2 and tmp.shape[-1] == 1):
                data[0]['label'] = add_dim_label(tmp)
    ...
    def train(epoch, program, ratio):
        for batch_id, data in enumerate(train_loader):
            ...
            #NOTE 3: before exe.run(), update the masks and parameters given the ratio and pruning mode (global or layer)
            pruner.uniform_prune(ratio, mode='global')
    ...
    if args.phase == 'train':
        ...
        incre_ratio = args.max_ratio - args.min_ratio
        last_ratio = args.min_ratio
        for i in range(args.resume_epoch, args.num_epochs):
            #NOTE 2: calculate the current pruning ratio according to the epoch number.
            if i <= args.num_epochs / float(2):
                ratio = incre_ratio * (4 / float(args.num_epochs**2)) * i
            else:
                ratio = incre_ratio * (4 / float(args.num_epochs) -
                                       (4 / float(args.num_epochs**2)) * i)
            last_ratio += ratio
            train(i, train_program, last_ratio)
            print("The current density of the inference model is {}%".format(
                round(100 * pruner.total_sparse(
                    paddle.static.default_main_program()), 2)))
            if i % args.test_period == 0:
                #NOTE 4: enforce zeros onto parameters according to pruner.masks. It's used to eliminate the generated zeros from the optimization step.
                pruner.update_params()
                test(i, val_program)
            if i > args.resume_epoch and i % args.model_period == 0:
                #NOTE 5: the same to STEP2
                pruner.update_params()
